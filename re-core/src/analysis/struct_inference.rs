...
use crate::il::llil::{LlilExpr, LlilStmt, BinOp};
...
impl AnalysisPass for StructInferencePass {
    ...
    fn run_analysis(
        ...
        for func in functions.functions.values() {
            ...
            let Ok(insns) = disasm.disassemble_range(memory, func.start_address, size.min(0x1000)) else {
                continue;
            };

            let llil = match self.arch {
                Architecture::Arm64 => crate::il::lifter_arm64::lift_function(&func.name, func.start_address, &insns),
                Architecture::Mips | Architecture::Mips64 => crate::il::lifter_mips::lift_function(&func.name, func.start_address, &insns),
                Architecture::RiscV32 | Architecture::RiscV64 => crate::il::lifter_riscv::lift_function(&func.name, func.start_address, &insns),
            ...
            for inst in &llil.instructions {
                for stmt in &inst.stmts {
                    match stmt {
                        LlilStmt::Store { addr, .. } => {
                            if let LlilExpr::BinOp { op: BinOp::Add, left, right } = &llil.exprs[*addr] {
                                if let (LlilExpr::Reg(r), LlilExpr::Const(c)) = (&llil.exprs[*left], &llil.exprs[*right]) {
                                    let entry = reg_max_offsets.entry(r.clone()).or_insert(0);
                                    if *c > *entry {
                                        *entry = *c;
                                    }
                                } else if let (LlilExpr::Const(c), LlilExpr::Reg(r)) = (&llil.exprs[*left], &llil.exprs[*right]) {
                                    let entry = reg_max_offsets.entry(r.clone()).or_insert(0);
                                    if *c > *entry {
                                        *entry = *c;
                                    }
                        ...
                        LlilStmt::SetReg { src, .. } => {
                            if let LlilExpr::Load { addr, .. } = &llil.exprs[*src]
                                && let LlilExpr::BinOp { op: BinOp::Add, left, right } = &llil.exprs[*addr]
                            {
                                if let (LlilExpr::Reg(r), LlilExpr::Const(c)) = (&llil.exprs[*left], &llil.exprs[*right]) {
                                    let entry = reg_max_offsets.entry(r.clone()).or_insert(0);
                                    if *c > *entry {
            ...
            for (reg, max_offset) in reg_max_offsets {
                ...
                if (8..0x10000).contains(&max_offset) && !reg.ends_with("sp") && !reg.ends_with("bp") {
                    findings.push(AnalysisFinding::new(
                        func.start_address,
                        FindingCategory::Info,
                        format!("Likely struct pointer in '{}': max offset 0x{:x}", reg, max_offset),
                        0.7,
                    ));
                }
            }
